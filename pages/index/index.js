...
var markerId;
var myLatitude;//我的位置 计算距离用
var myLongitude;//我的位置 计算距离用
...
wx.setStorageSync('LocationLatitude', 31.212592);//自己所在地
wx.setStorageSync('LocationLongitude', 121.609184);//自己所在地
...
Page({
  ...
  onLoad:function(e)
  {
    ...
    wx.setStorageSync('searchStr',"");//搜索的字符串 设置为空
      ...
      //     })
      //     wx.setStorageSync('LocationLatitude', res.latitude);//自己所在地
      //     wx.setStorageSync('LocationLongitude', res.longitude);//自己所在地
      //     that.setData({
      //       Vlatitude:res.latitude,
      //       Vlongitude:res.longitude
      //     })  
      //   },
      //   fail:function(res){
      //     console.log("定位失败1"+new Date().getTime())
      //     console.log(res);
      //     that.setData({
      //        Vlatitude:31.212592,
      //        Vlongitude:121.609184
      //      })
      //      wx.setStorageSync('LocationLatitude', 31.212592);//自己所在地
      //     wx.setStorageSync('LocationLongitude', 121.609184);//自己所在地          
      //   }
                      ...
                      success: function(res,ress) {
                        if(res.data.code==0)
                          ...
                          wx.setStorageSync('evcharAppkey', res.data.data);//缓存appkey
                                console.log("去登陆");
                                    wx.request({
                                url: 'https://wx.chongdian.club/login.php',
                                method:'POST',
                                data: {
                                ...
                                success: function(res) {     
                                  console.log("登陆结果")    
                                  console.log(res)        
                                  if(res.data.Edata[0].code!=0)//登陆失败
                                  {
                                  ...
                                  wx.setStorageSync('userID', res.data.Edata[0].data.id);//缓存用户id
                                  wx.setStorageSync('accessKey', res.data.Edata[0].data.accessKey);//缓存accessKey   加密信息用到
                                  wx.setStorageSync('accessToken', res.data.Edata[0].data.accessToken);//缓存accessToken
                                  wx.setStorageSync('userMobile', res.data.Edata[0].data.mobile);//缓存用户手机号
                                  wx.setStorageSync('userNickName', res.data.Edata[0].data.nickName);//缓存用户昵称
                                  console.log("昵称")
                                  console.log(wx.getStorageSync('userNickName'))
                                  //画出默认地图  开始
                                  console.log("onload112222")
                                  wx.getLocation({
                                  type: 'wgs84',
                                  success: function(res) {
                                    console.log("定位点2")
                                    ...
                                    wx.setStorageSync('myLatitude', res.latitude);//缓存我的位置
                                    wx.setStorageSync('myLongitude', res.longitude);//缓存我的位置
                                  ...
                                  fail:function(res){
                                    ...
                                    wx.setStorageSync('LocationLatitude', 31.212592);//自己所在地 上海长泰
                                    wx.setStorageSync('LocationLongitude', 121.609184);//自己所在地   上海长泰     
                                    that.drawMap()      
                                  }
                                })
                                  //画出默认地图  结束
  ...
  onShow: function () 
  {
    ...
    if(wx.getStorageSync('filter')==1)
    {
      ...
      console.log('{"appKey":"'+wx.getStorageSync('evcharAppkey')+'","areaCode":'+wx.getStorageSync('MareaCode')+',"deviceTypeList":'+wx.getStorageSync('MdeviceTypeList')+',"km":'+wx.getStorageSync('Mkm')+',"latitude":'+wx.getStorageSync('LocationLatitude')+',"longitude":'+wx.getStorageSync('LocationLongitude')+',"recommend":'+wx.getStorageSync('Mrecommend')+',"search":'+wx.getStorageSync('Msearch')+',"statusList":'+wx.getStorageSync('MstatusList')+'}')
      that.drawMap();
